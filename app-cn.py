...
with st.sidebar:
    ...
    first_malig_cn = st.selectbox("结肠癌是否为患者首发癌症（无其他恶性肿瘤病史）", ["是", "否"])
...
if st.sidebar.button("🚀 点击分析预后", type="primary"):
    ...
    # --- 重要逻辑对齐 ---
    # 唯一原发: 选是则 input为1 (One.primary.onlyYes)
    if primary_only_cn == "是": input_vec[13] = 1 
    # 首发肿瘤: 选是则结肠癌即为首发, 映射 First.malignant.pri.Yes = 1
